...
// 用于访问物理页引用计数数组
// 计算物理地址在引用数组中的索引--物理地址减去内核基地址除以页大小
#define PA2PGREF_ID(p) (((p)- KERNBASE)/PGSIZE) 

// 计算引用数组的最大
#define PGREF_MAX_ENTRIES PA2PGREF_ID(PHYSTOP)

struct spinlock pgreflock; // 用于 pageref 数组的锁，防止竞态条件引起的内存泄漏
...
kfree(void *pa)
{
  ...
  if (--PA2PGREF(pa) <= 0) {
    ...
    // pa will be memset multiple times if race-condition occured.
    memset(pa, 1, PGSIZE);
...
// A given physical page may be referred to by multiple processes' page tables, 
// and should be freed only when the last reference disappears.
// 当引用已经小于等于1时，不创建和复制到新的物理页，而是直接返回该页本身
void *kcopy_n_deref(void *pa) {
  acquire(&pgreflock);


  if (PA2PGREF(pa) <= 1) {  // 如果只有一个引用，无需复制
    release(&pgreflock);
    return pa;
  }

  // 分配新的物理页，并复制旧页中的数据到新页
