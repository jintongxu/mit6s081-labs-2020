...
// 内核页表中需要一些基本的映射，比如硬件、中断控制等等才能正常工作
...
kvm_free_kernelpgtbl(pagetable_t pagetable)
{
  ...
  for (int i = 0; i < 512; i++) {
    ...
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0) {  // 如果该页表指向更低一级的页表
      // 递归释放低一级页表及其页表项
...
kvminit()
{
  kernel_pagetable = kvminit_newpgtbl();   // 申请全局内核页表,用于内核 boot 过程以及无进行运行时使用 
}
