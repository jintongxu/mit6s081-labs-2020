...
// 在进程的16个vma槽中,找到可用的空槽,并且顺序计算所有vma中使用到的最低的虚拟地址
// (作为新 vma 结尾地址 vaend,开区间),然后将当前文件映射到该最低地址下面的位置(vastart = vaend - sz)
uint64
sys_mmap(void)
{
  ...
  // map the file
  // out implementation maps file right below where the trapframe is,
  // from high addressed to low addressed
...
sys_munmap(void)
{
  ...
  vmaunmap(p->pagetable, addr_aligned, nunmap, v);  // custom memory page unmap routine for mapped pages;
...
// if so, touch it so its mapped to an actual physical page and contains
